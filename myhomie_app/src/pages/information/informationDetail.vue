        ...
        <div class="detail" ref="detail">
            <div class="news_news">
                ...
                <div class="news_describe">
                    ...
                    <span class="time">{{postDetail.created_time2}}</span>
                </div>
        ...
        methods: {
            ...
            isPostFavored() {//初始化时判断帖子是否收藏
                this.$api.isFav(this.postComment).then(res => {
                    var status = res.data;
                    if (status == '1') {
                        this.isPostFavor = true;
                    } else {
                        this.isPostFavor = false;
                    }
                });
            },
            ...
            getPostDetail() {//初始化获得帖子详情
                this.$api.queryPostDetail(this.postComment + '&isView=1').then(res => {
                    ...
                    this.handleUserInfo(this.postDetail.userid);
                    this.postDetail.created_time2 = time_post_detail(this.postDetail.created_time);
