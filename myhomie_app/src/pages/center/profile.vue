<template>
	<section class="profile">
		...
		<div class="center_header">
	      ...
	      <van-uploader :after-read="insertImg" accept="image/gif, image/jpeg, image/jpg, image/png, image/svg" style="margin: 116px auto 10px;margin-left: 50%;transform: translate(-50%,0);">
		...
		<div class="item_list">
			<div class="item">
				...
				<input type="text" class="input_change" ref="input" placeholder="your nickname" maxlength="20" v-model.lazy="nickname">
			</div>
			<div class="item">
				<span class="left">{{$t('center.sign')}}</span>
				<input type="text" class="input_change" ref="input" placeholder="your signature" v-model.lazy="signature" maxlength="40">
			</div>
		</div>
		<div class="item_list">
			<div class="item">
			...
			<van-popup v-model="showSex" class="sexPopup">
				<van-radio-group v-model="radio">
				  <van-cell-group>
				    <van-cell :title="$t('center.male')" clickable @click="radio = this.$t('center.male');showSex = false">
				      <van-radio :name="$t('center.male')" checked-color="#fe8f2d"/>
				    </van-cell>
				    <van-cell :title="$t('center.female')" clickable @click="radio = this.$t('center.female');showSex = false">
			...
			<van-popup v-model="showNationality" class="sexPopup natPopup">
				<van-radio-group v-model="radio2">
				  <van-cell-group>
				    <van-cell :title="nat" v-for="(nat,index) in nationality" :key="index"clickable @click="selectNat(index)">
