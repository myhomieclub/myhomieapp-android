<template>
    <section>
        ...
        <div class="bottom_content">
            <input type="text" class="com2" readonly="readonly" :placeholder="$t('util.com')" @click="toComment()"
                   v-if="isPostComment">
            <input type="text" class="com_reply" readonly="readonly" :placeholder="$t('util.com')" @click="toReply()"
                   v-if="!isPostComment">
            <div v-if="isPostComment">
                ...
                <div class="right_item">
                    <i class="iconfont icon-favor" :class="{'icon-favorfill beyellow':isPostFavor}"
                       @click="favorPost()"></i>
                </div>
                <div class="right_item">
                    <i class="iconfont icon-appreciate" :class="{'icon-appreciatefill bered':isPostLike}"
                       @click="likePost()"></i>
                </div>
                <div class="right_item">
        ...
        <!--评论输入框-->
        <van-popup v-model="show" position="bottom" :overlay="true" overlay-class="overlay_color"
                   @click-overlay="closePopup" style="min-height:48px">
            <div class="comment_input">
                ...
                <div class="insert_img">
                    <van-uploader :after-read="insertImg"
        ...
        <van-actionsheet v-model="share" title="分享到">
            <div class="share_to">
                <div class="share_item" @click="shareTo('session')">
                    <van-icon name="friends-o"/>
                    微信好友
                </div>
                <div class="share_item" @click="shareTo('timeline')">
                ...
                this.$api.likePost(form).then(res => {
                    if (res.status_code == '1') {
                        if (res.like_status == '1') {
                            this.$emit('isPostLike', true);
                        } else {
                            this.$emit('isPostLike', false);
                        }
                    } else {
