	...
	<div class="search">
		...
		<div v-if="showSearchItem">
			...
			<div class="my_search">
				...
				<div class="search_item" v-show="index>0?my.item!=mySearch[index-1].item:my.item" v-for="(my,index) in mySearch" :key="index">
		...
		<div class="search_result left" v-if="!showSearchItem">
			<div class="null" style="width: 100%; height: 50px;line-height: 50px;font-size: 16px;text-align: center; color: #333;" v-show="noData">
            ...
            <!--搜索结果为信息-->
            <div class="information_list left" v-if="post_type==2" @click="goto(post.id)" v-for="(post,index) in list" :key="post.id">
	    ...
	    handlePostList3(postlist) {
	        for(let i=0; i<postlist.length; i++) {
	          ...
	          this.$api.isPostLiked(form).then(res => {
	            ...
	            else { this.$set(postlist[i], 'isPostLike', false); }
	          });
	          postlist[i].created_time2 = time_post_detail(postlist[i].created_time);
	          let con = postlist[i].content;
	          let urls = /('|")\/?(\b((?!https?|ftp|file:\/\/)\w)+\b)[-A-Za-z0-9+&@#/%?=~_|!:.;\\]+[-A-Za-z0-9+&@#/%=~_|]('|")/g;
